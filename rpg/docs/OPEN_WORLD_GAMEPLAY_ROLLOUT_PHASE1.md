...
> **Статус:** finalized implementation design for Phase 1 rollout.  
> Документ является источником проектных ограничений для текущей реализации: открытый мир получает базовое PvE и gathering-наполнение на существующих rails без телепортов, dungeon runtime, boss runtime или rare-spawn scheduler.
...
Из-за того, что **Тёмный лес** перенесён глубже, legacy alias и dark-forest semantic metadata должны быть едиными:

- `dark_forest -> westwild_n7`;
- dark-forest zone metadata, future dungeon hook, governance hook and flavor tags belong to `westwild_n7`;
- `westwild_n4` remains **Лиственная роща** and must not carry the old dark-forest semantic override.
...
# 16. Finalized implementation boundary

This repository implementation keeps Phase 1 deliberately narrow:

- normal open-world packages are represented by existing location `mobs` plus `world_spawn_profiles` with normal spawn instances;
- elite anchors are represented only by existing elite spawn profiles at the approved canonical locations;
- pressure-like nodes remain normal reward/source content;
- gathering surfaces are ordinary location `gather` entries that resolve through the gathering foundation;
- route, region, and flavor identity continue to resolve from canonical locations through reward/source metadata helpers;
- stubs remain stubs: South Coast is shoreline-lite, Old Mine is sparse mining plus cave fauna.

Future paper anchors remain non-runtime design notes only:

- Ashen future bosses: “Хранитель руин”, “Лич”;
- South Coast future boss candidate: “морской змей”.

They are intentionally not implemented as boss encounters, special boss spawns, dungeon encounters, or schedulers in Phase 1.
