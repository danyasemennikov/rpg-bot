...
class OpenWorldRouteTuningPass1PR3FTests(unittest.TestCase):
    def test_westwild_route_report_tuning_state(self):
        report = build_open_world_route_balance_report('route_westwild')

        self.assertEqual(report.get('route_id'), 'route_westwild')
    ...
    def test_south_coast_stub_report_tuning_state(self):
        report = build_open_world_route_balance_report('route_south_coast_stub')

        self.assertEqual(report.get('route_id'), 'route_south_coast_stub')
    ...
    def test_route_composition_and_live_content_remain_bidirectionally_aligned_for_tuned_routes(self):
        for route_id in ('route_westwild', 'route_south_coast_stub'):
    ...
    def test_documentation_guard(self):
        doc = Path(__file__).resolve().parent.parent.joinpath('docs', 'OPEN_WORLD_ROUTE_TUNING_PASS1.md')
        self.assertTrue(doc.exists())

        text = doc.read_text(encoding='utf-8').lower()
        self.assertIn('route_westwild', text)
        self.assertIn('route_south_coast_stub', text)
