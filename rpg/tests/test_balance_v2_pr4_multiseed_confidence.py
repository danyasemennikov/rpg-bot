...
def test_pr4_compact_pr3_baseline_preserved():
    ...
    assert compact["mob_pressure_lane"] == 35
...
def test_pr4_normalized_lane_interpretations_do_not_treat_seed_totals_as_raw_growth():
    ...
    assert mob_pressure["compact"] == 35
    assert mob_pressure["multiseed_total"] == 113
    assert mob_pressure["expected_multiseed_total"] == 105
    assert mob_pressure["delta_vs_expected_total"] == 8
    assert mob_pressure["multiseed_per_seed_avg"] == 37.67
    assert mob_pressure["delta_per_seed_vs_compact"] == 2.67
...
def test_pr4_checked_in_report_contains_section_and_pr3_counts():
    ...
    assert "Current mob_pressure_lane count: 35." in content
    assert "Current route_expectation_lane count: 44." in content
    assert "Current bad_matchup_review_lane count: 1." in content
    assert "Compact PR3 lane counts: mob_pressure_lane=35, route_expectation_lane=44, bad_matchup_review_lane=1" in content
