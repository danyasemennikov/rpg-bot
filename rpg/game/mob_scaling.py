...
PR3_LATE_STAGE_MOB_PRESSURE_REFINEMENTS: dict[tuple[str, str], dict[str, float]] = {
    # Balance V2 PR3: bounded simulation/reporting-only refinements for
    # PR2 mob_pressure_lane late-stage pressure-role clusters. These are
    # intentionally route-stage scoped and do not touch live mob templates.
...
def build_scaled_mob_stats(base_mob: dict, *, encounter_level: int, mob_role: str, route_id: str | None = None, stage: str | None = None) -> dict:
    ...
    route_stage_key = (str(route_id or ""), stage_key)
    targeted_override = TARGETED_ROUTE_STAGE_PRESSURE_OVERRIDES.get(route_stage_key, {})
    pr3_refinement = PR3_LATE_STAGE_MOB_PRESSURE_REFINEMENTS.get(route_stage_key, {})
    final_stage_modifier = {
        k: stage_modifier.get(k, 1.0) * float(targeted_override.get(k, 1.0)) * float(pr3_refinement.get(k, 1.0))
        for k in SCALED_MOB_STAT_KEYS
    }
    ...
    scaled["scale_components"] = {
        ...
        "pr3_late_stage_mob_pressure_refinement": pr3_refinement,
